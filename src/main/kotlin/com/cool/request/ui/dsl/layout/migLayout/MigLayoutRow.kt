...
internal class MigLayoutRow(
    ...
    private val incrementsIndent: Boolean = parent != null
) : Row() {
    companion object {
        ...
        private fun configureSeparatorRow(row: MigLayoutRow, @NlsContexts.Separator title: String?) {
            val separatorComponent =
                if (title == null) SeparatorComponent(0, OnePixelDivider.BACKGROUND, null) else TitledSeparator(title)
    ...
    private fun createBlockRow(@NlsContexts.Separator title: String?, isSeparated: Boolean, init: Row.() -> Unit): Row {
        val parentRow =
            createChildRow(indent = indent, title = title, isSeparated = isSeparated, incrementsIndent = isSeparated)
        parentRow.init()
    ...
    override fun hideableRow(title: String, init: Row.() -> Unit): Row {
        val titledSeparator = com.cool.request.ui.dsl.layout.HideableTitledSeparator(title)
    ...
    internal fun addComponent(component: JComponent, cc: CC = CC()) {
        ...
        if (labeled && components.size == 2 && component.border is LineBorder) {
            builder.componentConstraints.get(components.first())?.vertical?.gapBefore =
...
private class CellBuilderImpl<T : JComponent> internal constructor(
    ...
    override fun withValidationOnInput(callback: ValidationInfoBuilder.(T) -> ValidationInfo?): CellBuilder<T> {
        builder.componentValidateCallbacks[component.origin] =
            { callback(ValidationInfoBuilder(component.origin), component) }
        property?.let {
            builder.customValidationRequestors.getOrPut(component.origin, { SmartList() }).add(it::afterPropagation)
        }
        return this
    }
    ...
    override fun withLargeLeftGap(): CellBuilder<T> {
        builder.updateComponentConstraints(component) {
            horizontal.gapBefore =
                gapToBoundSize(builder.spacing.largeHorizontalGap, true)
        }
        return this
    }
